...
int PvP = Integer.parseInt(request.getParameter("pvp_id") == null ? "0" : request.getParameter("pvp_id"));

if (PvP > 0) {
	delstatus = service.deletePVP(PvP, request,config);
	if (delstatus == true) {
		response.sendRedirect("../manage_personal_vechile_party.jsp?msg=YesDel");
	} else {
		response.sendRedirect("../manage_personal_vechile_party.jsp?msg=NoDel");
	}
}
...
// Delete Transpoter
int Transpoter = Integer.parseInt(request.getParameter("Transpoter_id") == null ? "0" : request.getParameter("Transpoter_id"));

if (Transpoter > 0) {
	delstatus = service.deleteTranspoter(Transpoter, request,config);
	if (delstatus == true) {
		response.sendRedirect("../manage_transpoter.jsp?msg=YesDel");
	} else {
		response.sendRedirect("../manage_transpoter.jsp?msg=NoDel");
	}
}
...
int District_id = Integer
	.parseInt(request.getParameter("District_id") == null ? "0" : request.getParameter("District_id"));

if (District_id > 0) {
delstatus = service.deleteDistrict(District_id, request, config);
if (delstatus == true) {
	response.sendRedirect("../manage_district.jsp?msg=YesDel");
} else {
	response.sendRedirect("../manage_district.jsp?msg=NoDel");
}
}

//Delete INSTOCK

int inSTOCK_id = Integer
	.parseInt(request.getParameter("inStock_id") == null ? "0" : request.getParameter("inStock_id"));

if (inSTOCK_id > 0) {
delstatus = service.deleteInstock(inSTOCK_id, request, config);
if (delstatus == true) {
	response.sendRedirect("../manage_inStock.jsp?msg=YesDel");
} else {
	response.sendRedirect("../manage_inStock.jsp?msg=NoDel");
}
}
...
//Delete Billty
int billty_id = Integer.parseInt(request.getParameter("billty_id") == null ? "0" : request.getParameter("billty_id"));

if (billty_id > 0) {
	delstatus = service.deleteBillty(billty_id, request,config);
	if (delstatus == true) {
		System.out.println("Ajax Delete : "+delstatus);
		
		response.sendRedirect("../manage_billty.jsp?msg=YesDel");
	} else {
		response.sendRedirect("../manage_billty.jsp?msg=NoDel");
	}
}
//Delete Company Billty
int Company_billty_id = Integer.parseInt(request.getParameter("company_billty_id") == null ? "0" : request.getParameter("company_billty_id"));

if (Company_billty_id > 0) {
	delstatus = service.deleteCompanyBillty(Company_billty_id, request,config);
	if (delstatus == true) {
		System.out.println("Ajax Delete : " + delstatus);
		
		response.sendRedirect("../manage_company_billty.jsp?msg=YesDel");
	} else {
		response.sendRedirect("../manage_company_billty.jsp?msg=NoDel");
	}
}
...
if (spend_head_id > 0) {
    ...
    int product_id = Integer
		.parseInt(request.getParameter("product_id") == null ? "0" : request.getParameter("product_id"));

if (product_id > 0) {
	delstatus = service.deleteProductInfo(product_id, request, config);
	if (delstatus == true) {
		response.sendRedirect("../manage_product.jsp?msg=YesDel");
	} else {
		response.sendRedirect("../manage_product.jsp?msg=NoDel");
	}
}
...
if (vehicle_id > 0) {
	...
	if (delstatus == true) {
		response.sendRedirect("../manage_vehicle.jsp?msg=YesDel");
	} else {
		response.sendRedirect("../manage_vehicle.jsp?msg=NoDel");
	}
}
